...
% ----------------------------------------------------------------------- %
function table2latex(T, filename, ndecimal_digits)
    ...
    try
        for row = 1:size(T,1)
            ...
            for col = 1:n_col
                ...
                temp_var = num2str(value);
                % added for short display of decimal digits
                if nargin == 3
                    if isfloat(value), temp_var = temp_var(1:ndecimal_digits+2); end
                end
                temp{1,col} = temp_var;
            end
            if ~isempty(row_names)
