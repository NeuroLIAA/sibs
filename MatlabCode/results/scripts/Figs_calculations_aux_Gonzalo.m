...
% load trials data -> struct name: info_per_subj_final
trials_tmp                   = load(strcat(src_path, 'info_all_subj.mat'));

% load subjects variables
[ids_subjs, ~, subj_order]  = unique({trials_tmp.info_per_subj_final(:).subj});
subj_order                  = subj_order';
Nsubj                       = length(unique(subj_order));
Ntr                         = length(trials_tmp.info_per_subj_final);
...
Nimg                          = length(unique(ids_images));

% reduce scanpaths to grid and create struct: trials
delta                         = 32;
grid_size                     = image_size/delta;
trials                        = reduce_scanpaths(trials_tmp.info_per_subj_final, delta, image_size);
clear trials_tmp

% set fixations limits
min_fix     = 2;
max_fix     = 13;

% instantiate multimatch values
...
for ind_img=1:Nimg
    ...
    for subj_i=1:length(info_per_img)
        ...
        if ind_subj_target_found(subj_i)
            ...
            for subj_j=subj_i:length(info_per_img)
                if subj_j ~= subj_i
