...
%% Load Saliency
rng(1);
% cantidad de imagenes para mirar
Nsamp = 134;
sample = unidrnd(Nimg,[1,Nsamp]);
sampleNeg = unidrnd(Nimg,[1,20]);
map = 'deepgaze'; %{'mlnet', 'sam_resnet', 'sam_vgg', 'deepgaze', 'icf', 'humans', 'center'}
addpath('utils/saliency_mit')
addpath('utils/saliency_mit/code_forMetrics')
features = loadFixations(map, sample, images);
baselineMap = cast(imread('../saliency/center.jpg'),'double');
...
% %% Evaluación - Radio 0 - Fix 2 a 3
% 
% load(sprintf('../new_data/new_matrix/fixated_area_fix_%d_to_%d_radii_%d.mat', 2, 3, 0))
% clear fixatedAreaTrain fixatedAreaTest
% metrics = [];
% modelos = {'sam_vgg','deepgaze','icf','center'};
% otherMap = sum(fixatedArea(:,:,sampleNeg),3);
% for mod_ind = 1:length(modelos)
%    metrics.(modelos{mod_ind}) = evalSaliencyMap(modelos{mod_ind}, sample, ...
%        images, fixatedArea, baselineMap, otherMap, 1);
% end
% 
% %% Evaluación - Radio 1 - Fix 2 a 3
% load(sprintf('../new_data/new_matrix/fixated_area_fix_%d_to_%d_radii_%d.mat', 2, 3, 1))
% clear fixatedAreaTrain fixatedAreaTest
% metrics = [];
% modelos = {'sam_vgg','deepgaze','icf','center'};
% otherMap = sum(fixatedArea(:,:,sampleNeg),3);
...
%% Evaluación - Radio 0 - Fix 3
load(sprintf('../new_data/new_matrix/fixated_area_fix_%d_to_%d_radii_%d.mat', 3, 3, 0))
clear fixatedAreaTrain fixatedAreaTest
metrics = [];
modelos = {'mlnet','sam_resnet','sam_vgg','deepgaze','icf','center'};
otherMap = sum(fixatedArea(:,:,sampleNeg),3);
for mod_ind = 1:length(modelos)
   metrics.(modelos{mod_ind}) = evalSaliencyMap(modelos{mod_ind}, sample, ...
       images, fixatedArea, baselineMap, otherMap, 1);
...
%% Evaluación - Radio 1 - Fix 3
load(sprintf('../new_data/new_matrix/fixated_area_fix_%d_to_%d_radii_%d.mat', 3, 3, 1))
clear fixatedAreaTrain fixatedAreaTest
metrics = [];
modelos = {'sam_vgg','deepgaze','icf','center'};
otherMap = sum(fixatedArea(:,:,sampleNeg),3);
...
%     otherMap = double(sum(features(:,:,indrand))/255);
%     
