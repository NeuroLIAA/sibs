...
%     end
    ...
    tmp.info_per_subj =  reduce_scanpaths( tmp.info_per_subj, delta, image_size );
    info_per_subj = [info_per_subj tmp.info_per_subj];
end
...
%% F) Spatial distributions of the th fixation 
...
figure(100);clf
    ...
    axes('Position',[0.050, 0.850, 0.1, 0.1])
        hold on
            im = imread(['../../data_images/templates/' info_per_subj(1).target_name]);
            [z1,z2] = size(im);
            tmp     = 128*ones(sy,sx);
    ...
    axes('Position',[0.170, 0.700, 0.1, 0.1])
        hold on
            tr = 1;
            im = imread(['../../data_images/images/' info_per_subj(tr).image_name]);
...
%     xh = 0:20:1000;
    ...
    for i = 1:length(FIXRANK)
        axes('Position',[0.05+0.075*(i-1), 0.05, 0.0725, 0.15])
            ...
            set(gca,'YDir','reverse')
            set(gca,'Visible','off')
            xlabel(strcat('Fix: ', string(FIXRANK(i))))
    end
    
%% Altogether figs: Version 2
figure(101);clf
    set(gcf,'Color','w')
    set(gcf,'Position',[66 1 1855 1001])
...
%     xh = 0:20:1000;
    ...
    for i = 1:length(FIXRANK)
        axes('Position',[0.05+0.075*(i-1), 0.05, 0.0725, 0.15])
        %title(strcat('Fix: ', string(FIXRANK(i))))
        hold on
            xlabel(strcat('Fix: ', string(FIXRANK(i))), 'FontSize',12)
            %if (i==1)
                    text(sx/2, sy,'Fixation position','HorizontalAlignment','center','FontSize',12); 
                %end
                pcolor(1-squeeze(MG(i,:,:))); colormap gray
                plot([1 sgx],[1 1],  '-','Color',cols(i,:),'LineWidth',3)
                plot([1 sgx],[sgy sgy],'-','Color',cols(i,:),'LineWidth',3)
